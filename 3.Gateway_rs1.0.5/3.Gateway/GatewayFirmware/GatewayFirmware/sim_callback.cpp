...
{
  sim_getdata(sim_sms_receive.msg_content);
...
extern uint8_t N1pumpstae;


void sim_getdata(char str[])
{

  Serial.println("Get data from SIM: ");
//  Serial.println("--");
  // Lấy ID
  char ID[3] ;
  int id = 0;
  ...
  if ((Type[0] == 'T')&&(Type[1] == 'H'))
  {
    ...
    N1pumpstae  = N1pumpstae;
    if (node_is_exist(id, TYPE_TH) == false)
    {
      nodeth_add(id, Temp, Humidity, GWpumpon, N1pumpstae, false);
    }
    else
    {
      nodeth_setvalue(id, Temp, Humidity, GWpumpon, N1pumpstae, false);
    }
  }
  ...
  if ((Type[0] == 'D')&&(Type[1] == 'P'))
  {
    ...
    Serial.println(N1pumpon);
     ...
     if (node_is_exist(id, TYPE_DP) == false)
      {
        nodedp_add(id, Distance, PH, GWpumpon, N1pumpon, false);
      }
      else
      {
        nodedp_setvalue(id, Distance, PH, GWpumpon, N1pumpon, false);
      }
  }
}
...
void sim_serial_init(void) 
{
//  Serial.print("COM1- Tx: ");
//  Serial.print(TX1);
//  Serial.print("   RX:");
//  Serial.println(RX1);
  Serial1.begin(19200, SERIAL_8N1, RX1, TX1); 
};
...
void sim_receive_process(void)
{
  ...
  while (Serial1.available())
  {
    ...
    if (str.length() > 0)
    {
      for (int i = 0; i< str.length(); i++) sim_data_buffer.push(str[i]);
      //Serial.print(str);
    }
  }
	
	while ((c=sim_data_buffer.pop()) >0)
	{
		if (c != 0)
		{
